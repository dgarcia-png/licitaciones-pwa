...
export default function DecisionesPage() {
  ...
  useEffect(() => {
    const cargar = async () => {
      setCargando(true)
      try {
        // Batch: oportunidades + datos base en una sola llamada
  ...
  useEffect(() => {
    ...
    const cargarOpo = async () => {
      setCargando(true)
      try {
        // 1 llamada batch en lugar de 5 llamadas individuales
  ...
  const candidatas = oportunidades.filter(o => ['en_analisis', 'go', 'no_go'].includes(o.estado))
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              {ac.riesgos_detectados?.length > 0 && (
                <div className="bg-red-50 border border-red-200 rounded-2xl p-5">
                  ...
                  {ac.riesgos_detectados.map((r: string, i: number) => <p key={i} className="text-xs text-red-700 mb-2">• {r}</p>)}
                </div>
              )}
              {ac.oportunidades_detectadas?.length > 0 && (
                <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5">
                  ...
                  {ac.oportunidades_detectadas.map((o: string, i: number) => <p key={i} className="text-xs text-emerald-700 mb-2">• {o}</p>)}
                </div>
              )}
            </div>
          )}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    <div>
                      ...
                      <select value={escenarioElegido} onChange={e => setEscenarioElegido(e.target.value)}
                        ...
                        <option value="">— Seleccionar —</option>
                        <option value="conservador">Conservador {res.precioConservador ? '(' + fmt(res.precioConservador) + ')' : ''}</option>
                        <option value="objetivo">Recomendado {res.precioObjetivo ? '(' + fmt(res.precioObjetivo) + ')' : ''}</option>
                        <option value="competitivo">Competitivo {res.precioCompetitivo ? '(' + fmt(res.precioCompetitivo) + ')' : ''}</option>
                    ...
                    <div>
                      ...
                      <input type="number" step="0.01" value={precioAprobado} onChange={e => setPrecioAprobado(e.target.value)}
                        placeholder={res.precioObjetivo ? String(Math.round(res.precioObjetivo * 100) / 100) : '0'}
                        className="w-full mt-1 px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white focus:ring-2 focus:ring-purple-500 focus:outline-none" />
                    </div>
                  </div>
                  <div className="mb-3">
