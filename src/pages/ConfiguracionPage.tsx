...
export default function ConfiguracionPage() {
  const [tab, setTab] = useState<'filtros' | 'convenios' | 'costes'>('filtros')
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      for (const conv of (convData.convenios || [])) { const catData = await api.categoriasConvenio(conv.id); cats[conv.id] = catData.categorias || [] }
      setCategorias(cats)
  ...
  const handleAddCoste = async () => {
    if (!nuevoItem || !nuevoItem.bloque || !nuevoItem.concepto) { setError('Bloque y concepto obligatorios'); return }
    setGuardando('nuevo')
    try { await api.addCosteRef({ bloque: nuevoItem.bloque, concepto: nuevoItem.concepto, unidad: nuevoItem.unidad, valor: parseFloat(nuevoItem.valor) || 0, notas: nuevoItem.notas, activo: true }); setNuevoItem(null); setMensaje('Añadido'); await cargarDatos() }
    catch (e) { setError('Error') } finally { setGuardando('') }
  }
  ...
  const handleAddConfig = async () => {
    if (!nuevoConfig || !nuevoConfig.tipo || !nuevoConfig.valor) { setError('Tipo y valor obligatorios'); return }
    setGuardando('cfgnuevo')
    try { await api.addConfig({ tipo: nuevoConfig.tipo, valor: nuevoConfig.valor, activo: true, descripcion: nuevoConfig.descripcion }); setNuevoConfig(null); setMensaje('Añadido'); await cargarDatos() }
    catch (e) { setError('Error') } finally { setGuardando('') }
  }
  ...
  return (
    <div className="p-6 lg:p-8 max-w-4xl">
      <div className="flex items-center gap-4 mb-6">
        <div className="p-2.5 bg-gradient-to-br from-slate-600 to-slate-800 rounded-xl shadow-lg shadow-slate-200"><Settings size={22} className="text-white" /></div>
        <div><h1 className="text-2xl font-bold text-slate-900">Configuración</h1><p className="text-sm text-slate-500">Filtros, convenios y costes de referencia</p></div>
      </div>

      {/* Tabs */}
      <div className="flex gap-2 mb-6">
        <button onClick={() => setTab('filtros')} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-colors ${tab === 'filtros' ? 'bg-slate-900 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}><Filter size={16} /> Filtros PASO 0</button>
        <button onClick={() => setTab('convenios')} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-colors ${tab === 'convenios' ? 'bg-slate-900 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}><BookOpen size={16} /> Convenios ({convenios.length})</button>
        <button onClick={() => setTab('costes')} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-colors ${tab === 'costes' ? 'bg-slate-900 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}><Euro size={16} /> Costes ref.</button>
      </div>
        ...
        <>
          <div className="bg-white border border-slate-200 rounded-2xl p-5 mb-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setNuevoConfig({ tipo: 'CPV', valor: '', descripcion: '' })}
                className="flex items-center gap-1 px-3 py-2 text-xs font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-xl"><Plus size={14} /> Añadir</button>
            </div>
          </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5 mb-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <select value={nuevoConfig.tipo} onChange={e => setNuevoConfig({ ...nuevoConfig, tipo: e.target.value })}
                ...
                <div>
                  ...
                  <input type="text" value={nuevoConfig.valor} onChange={e => setNuevoConfig({ ...nuevoConfig, valor: e.target.value })}
                    placeholder={TIPOS_CONFIG[nuevoConfig.tipo]?.placeholder || ''} className="w-full px-2 py-1.5 border border-slate-200 rounded-lg text-xs" />
                </div>
                <div>
                  <label className="text-[10px] text-slate-500">Descripción</label>
                  <input type="text" value={nuevoConfig.descripcion} onChange={e => setNuevoConfig({ ...nuevoConfig, descripcion: e.target.value })}
                    className="w-full px-2 py-1.5 border border-slate-200 rounded-lg text-xs" />
                </div>
              </div>
              <div className="flex gap-2 mt-3">
                <button onClick={handleAddConfig} disabled={guardando === 'cfgnuevo'}
                  className="flex items-center gap-1 px-4 py-2 text-xs font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg disabled:bg-blue-400">
                  {guardando === 'cfgnuevo' ? <Loader2 size={12} className="animate-spin" /> : <Save size={12} />} Guardar
                </button>
          ...
          {GRUPOS_CONFIG.map(grupo => {
            const items = configItems.filter(i => grupo.tipos.includes(i.tipo))
            if (items.length === 0) return null
            // Sub-group by tipo within grupo
            const subgrupos: Record<string, any[]> = {}
            items.forEach(i => { if (!subgrupos[i.tipo]) subgrupos[i.tipo] = []; subgrupos[i.tipo].push(i) })
            return (
              <Bloque key={grupo.titulo} title={grupo.titulo} badge={`${items.filter(i => i.activo).length}/${items.length}`} defaultOpen={grupo.titulo === 'Filtros principales'}>
                {Object.entries(subgrupos).map(([tipo, tipoItems]) => {
                  const info = TIPOS_CONFIG[tipo] || { label: tipo, icon: Hash, color: 'bg-slate-100 text-slate-700', placeholder: '' }
                  const TipoIcon = info.icon
                  return (
                    <div key={tipo} className="mb-4 last:mb-0">
                      <div className="flex items-center gap-2 mb-2">
                        <span className={`inline-flex items-center gap-1 text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${info.color}`}>
                          <TipoIcon size={10} /> {info.label}
                        </span>
                        ...
                        {tipoItems.map((item: any) => (
                          <div key={item.fila} className={`flex items-center gap-2 p-2 rounded-lg ${item.activo ? 'bg-white' : 'bg-slate-50 opacity-60'}`}>
                            <button onClick={() => handleToggleConfig(item.fila, item.activo)} disabled={guardando === 'cfg' + item.fila} className="shrink-0">
                              {guardando === 'cfg' + item.fila ? <Loader2 size={14} className="animate-spin text-slate-400" /> :
                                item.activo ? <ToggleRight size={16} className="text-emerald-600" /> : <ToggleLeft size={16} className="text-slate-400" />}
                            </button>
        ...
        <>
          <div className="bg-white border border-slate-200 rounded-2xl p-5 mb-4">
            ...
            <label className={`flex flex-col items-center justify-center p-6 border-2 border-dashed rounded-xl cursor-pointer transition-all ${subiendo ? 'border-blue-300 bg-blue-50' : 'border-slate-300 hover:border-blue-400 hover:bg-blue-50/50'}`}>
              {subiendo ? (<><Loader2 size={24} className="text-blue-500 animate-spin mb-2" /><span className="text-sm text-blue-700 font-medium">Procesando con Gemini...</span></>)
              : (<><Upload size={24} className="text-slate-400 mb-2" /><span className="text-sm text-slate-600 font-medium">Seleccionar PDF del convenio</span></>)}
              <input type="file" accept=".pdf" onChange={handleSubirConvenio} disabled={subiendo} className="hidden" />
            </label>
          </div>
          {convenios.length === 0 ? (
            <div className="flex flex-col items-center py-12"><BookOpen size={40} className="text-slate-300 mb-3" /><p className="text-slate-500">No hay convenios cargados</p></div>
            ...
            return (
              <Bloque key={conv.id} title={`${conv.sector} — ${conv.provincia}`} badge={`${cats.length} cat.`} defaultOpen={false}
                actions={<button onClick={() => handleEliminarConvenio(conv.id, `${conv.sector} ${conv.provincia}`)} disabled={guardando === conv.id}
                  className="flex items-center gap-1 px-3 py-1.5 text-xs text-red-600 bg-red-50 hover:bg-red-100 rounded-lg">
                  {guardando === conv.id ? <Loader2 size={12} className="animate-spin" /> : <Trash2 size={12} />} Eliminar</button>}>
        ...
        <>
          <div className="bg-white border border-slate-200 rounded-2xl p-5 mb-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setNuevoItem({ bloque: bloques[0] || '', concepto: '', unidad: '', valor: '0', notas: '' })}
                className="flex items-center gap-1 px-3 py-2 text-xs font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-xl"><Plus size={14} /> Añadir ítem</button>
            </div>
          ...
          {nuevoItem && (
            <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5 mb-4">
              ...
              <div className="flex gap-2 mt-3">
                <button onClick={handleAddCoste} disabled={guardando === 'nuevo'} className="flex items-center gap-1 px-4 py-2 text-xs font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg disabled:bg-blue-400">
                  {guardando === 'nuevo' ? <Loader2 size={12} className="animate-spin" /> : <Save size={12} />} Guardar</button>
  ...
  return (
    <tr className={`border-b border-slate-50 hover:bg-slate-50 ${item.activo ? '' : 'opacity-50'}`}>
      ...
      <td className="px-2 py-1.5 text-right">
        {editando ? (<div className="flex items-center gap-1 justify-end"><input type="number" step="any" value={valorTemp} onChange={e => setValorTemp(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Enter') guardar(); if (e.key === 'Escape') setEditando(false) }} className="w-20 px-2 py-0.5 border border-blue-300 rounded text-xs text-right" /><button onClick={guardar} className="text-emerald-600"><Save size={12} /></button><button onClick={() => setEditando(false)} className="text-slate-400"><X size={12} /></button></div>)
